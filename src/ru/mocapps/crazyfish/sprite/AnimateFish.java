...
public class AnimateFish extends AnimatedSprite {
	...
	public AnimateFish(float pX, float pY, ITiledTextureRegion pTiledTextureRegion, VertexBufferObjectManager vertexBufferObjectManager, Engine mEngine, String fishName) {
		...
		indicatorLife.registerEntityModifier(new SequenceEntityModifier(new ParallelEntityModifier(new AlphaModifier(10, 0.0f, 1.0f), new ScaleModifier(10, 0.5f, 1.0f)), new RotationModifier(5, 0, 360)
	...
	protected void onManagedUpdate(final float pSecondsElapsed) {
		try {
			this.setX(LogicController.getFish(fishName).getXPosition() - this.getWidth() / 2);
			this.setY(LogicController.getFish(fishName).getYPosition() - this.getHeight() / 2);
